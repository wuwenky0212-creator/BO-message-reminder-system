...
class DDLValidator:
    ...
    def validate_indexes(self) -> bool:
        ...
        expected_indexes = [
            ('uk_log_id', 'log_id', True),  # UNIQUE index
        ...
        for index_name, column_name, is_unique in expected_indexes:
            if is_unique:
                pattern = rf'CREATE\s+UNIQUE\s+INDEX.*{index_name}.*ON\s+audit_log\s*\(\s*{column_name}\s*\)'
            else:
                pattern = rf'CREATE\s+INDEX.*{index_name}.*ON\s+audit_log\s*\(\s*{column_name}\s*\)'
    ...
    def validate_comments(self) -> bool:
        ...
        # 检查表注释
        if re.search(r'COMMENT\s+ON\s+TABLE\s+audit_log', self.ddl_content, re.IGNORECASE):
            print("✓ 表注释已添加")
        ...
        # 检查字段注释
        comment_count = len(re.findall(r'COMMENT\s+ON\s+COLUMN\s+audit_log\.', self.ddl_content, re.IGNORECASE))
        if comment_count > 0:
    ...
    def validate_sql_syntax(self) -> bool:
        ...
        # 检查 CREATE TABLE 语句
        if re.search(r'CREATE\s+TABLE.*audit_log', self.ddl_content, re.IGNORECASE | re.DOTALL):
    ...
    def mock_insert_test(self) -> bool:
        ...
        # 模拟一条插入数据
        mock_data = {
            'log_id': 'log_20240115_143000_001',
            'event_type': 'REMINDER_PROCESSED',
            'rule_code': 'CHK_TRD_004',
            'user_id': 'user123',
            'user_name': '张三',
            'operation_type': 'MARK_AS_READ',
            'business_id': 'BIZ_20240115_001',
            'count_before': 15,
            'count_after': 14,
    ...
    def mock_query_test(self) -> bool:
        ...
        # 模拟常见查询场景
        query_scenarios = [
            ('按日志ID查询', 'log_id', 'uk_log_id'),
            ('按事件类型查询', 'event_type', 'idx_event_type'),
            ('按规则代码查询', 'rule_code', 'idx_rule_code'),
            ('按用户ID查询', 'user_id', 'idx_user_id'),
            ('按时间戳排序', 'timestamp', 'idx_timestamp')
        ]
...
def main():
    ...
    # DDL 文件路径
    ddl_file = 'database/ddl/audit_log.sql'
